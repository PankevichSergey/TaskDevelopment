...
void del() {
    ...
    while (right.size() > 1) {
        int g = gcd(right.back().first, left.back().second);
        left.push_back({right.back().first, g});
        right.pop_back();
    }
    left.pop_back();
}
...
int get() {
    return gcd(right.back().second, left.back().second);
}
